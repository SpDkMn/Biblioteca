...
<div class="alert alert-success alert-dismissible" role="alert" id="cuadro_identificacion_envio" style="margin-bottom: 0px">
<button type="button" class="close" data_dismiss="alert" aria-label="Close" id="identificacion_envio" onclick="cerrar_identificacion()"><span aria-hidden="true">&times;</span></button>
{{Session::get('message')}}
</div>
@endif
@section('content')
	  ...
	  width: 50px;
		...
		<div class="col-md-3" id="boton_crear_noticia" style="padding-left: 10px; margin-top: 7px;">
                      ...
                      <a href="http://127.0.0.1:8000/admin/noticias/create" class="btn btn-primary">Crear una nueva Noticia</a>
                   </div>
							...
							<div class="box">
								...
								<div class="box" style="margin-left:17%">
									<form action="{{ route('noticias.destroy',$variable[($i-1)*4]->id )}}" method="POST">
							              ...
							              {{ csrf_field() }}
							              <input type="submit" class="btn btn-danger btn-x" value="Eliminar">
		             				 </form>
	             				 </div>
							</div>
						</div>
